...
public class DeniedOrderTest extends ManagementBaseTest {
    ...
    @Test(description = "Kiểm thử admin từ chối đơn hàng đang ở trạng thái chờ xác nhận")
    @AllureId("Admin-06")
    @Epic("Chức năng Quản lý đơn hàng")
    @Feature("Từ chối đơn hàng")
    @Story("Admin từ chối đơn hàng đang ở trạng thái chờ xác nhận")
    @Severity(SeverityLevel.CRITICAL)
    public void deniedOrder() throws InterruptedException {
        ...
        String confirmedOrderId = managementPage.deniedFirstPendingOrder("Hết món");
        ...
        profilePage.clickOrderTabByIndex(7);
        Thread.sleep(1000);
        // Kiểm tra đơn hàng có tồn tại
        boolean isFound = profilePage.isOrderIdVisibleInTab("Từ chối", confirmedOrderId);
